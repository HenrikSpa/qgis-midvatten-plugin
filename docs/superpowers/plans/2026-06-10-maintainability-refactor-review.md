...
## 13. Extract GUI classes from midvatten_utils — **DONE (move phase) 2026-06-12** (pure move to tools/utils/plot_templates.py; midvatten_utils 1479→585 lines; select_files called module-qualified so the 96 existing mock targets keep intercepting; no re-export needed — midv_addons verified not using the classes; side win: importing midvatten_utils no longer loads matplotlib.pyplot (~0.3s) on non-plot paths. REMAINING (second slice per the original two-step plan): unify the PlotTemplates/MatplotlibStyles import/parse/save/load duplication behind a shared base. NOTE found during contract check: midv_addons references midvatten_utils.{unicode_, rstrip, create_dict_from_db} which do NOT exist — pre-existing breakage on midv_addons' side, not from this slice)

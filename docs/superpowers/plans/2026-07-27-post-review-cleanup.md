...
- **Never change existing test reference data or assertion values.** Only Task 9 edits an existing test, and there the asserted *values* stay byte-identical — only the API being called changes. No other task may edit an existing test; Task 17 in particular must leave the two cursor tests from commit 55b51e9 untouched.

...
- **The dialog stays on top of the editor (only).** It is created as a `Qt.Tool` window parented to the editor (`self.setWindowFlags(self.windowFlags() | Qt.Tool)`), so it floats above the editor while the user clicks the plot and the From/To widgets underneath, but does NOT float above QGIS or other applications, and it hides when the user switches away from the editor. (Not `WindowStaysOnTopHint`, which would force it above everything.)
...
- **Tool-window flag** — assert the dialog is a `Qt.Tool` window (`(editor._resolve_dialog.windowFlags() & Qt.Tool) == Qt.Tool`) and parented to the editor (actual window stacking isn't meaningfully assertable in the headless test env, so we verify the flag + parent, not the pixels).

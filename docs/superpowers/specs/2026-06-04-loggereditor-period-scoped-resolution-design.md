...
- **The dialog stays on top.** It is created with the `Qt.WindowStaysOnTopHint` window flag (set in `__init__` via `self.setWindowFlags(self.windowFlags() | Qt.WindowStaysOnTopHint)`), so it floats above the editor while the user clicks the plot and the From/To widgets underneath — it never hides behind the main window.
...
- **Stays-on-top flag** — assert `bool(editor._resolve_dialog.windowFlags() & Qt.WindowStaysOnTopHint)` is set (actual window stacking isn't meaningfully assertable in the headless test env, so we verify the flag, not the pixels).

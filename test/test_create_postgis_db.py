...
class TestFillDb(utils_for_tests.MidvattenTestPostgisNotCreated):
    ...
    @mock.patch("qgis.utils.iface")
    @mock.patch("midvatten.midvatten_plugin.NewPostgisDbDialog")
    def test_create_db_locale_en(self, mock_dialog_cls, mock_iface):
        mock_dlg = mock.MagicMock()
        mock_dialog_cls.return_value = mock_dlg
        mock_dlg.exec.return_value = 1
    ...
    @mock.patch("qgis.utils.iface")
    @mock.patch("midvatten.midvatten_plugin.NewPostgisDbDialog")
    def test_create_db_setup_string(self, mock_dialog_cls, mock_iface):
        """
    ...
    @mock.patch("qgis.utils.iface")
    @mock.patch("midvatten.midvatten_plugin.NewPostgisDbDialog")
    def test_about_db_creation(self, mock_dialog_cls, mock_iface):
        """
    ...
    @mock.patch("qgis.utils.iface")
    @mock.patch("midvatten.midvatten_plugin.NewPostgisDbDialog")
    def test_about_db_creation_version_string(self, mock_dialog_cls, mock_iface):
        """
...
class TestObsPointsTriggers(utils_for_tests.MidvattenTestPostgisDbSv):
    def setup_method(self):
        with mock.patch(
            ...
            db_utils.sql_alter_db(
                """DROP TRIGGER IF EXISTS trigger_after_update_obs_points_geom_fr_coords ON obs_points;"""
            )
            db_utils.sql_alter_db(
                """DROP TRIGGER IF EXISTS trigger_after_insert_obs_points_coords_fr_geom ON obs_points;"""
            )
            db_utils.sql_alter_db(
                """DROP TRIGGER IF EXISTS trigger_after_update_obs_points_coords_fr_geom ON obs_points;"""
            )
            db_utils.sql_alter_db(
                """DROP FUNCTION IF EXISTS after_insert_obs_points_geom_fr_coords();"""
            )
            db_utils.sql_alter_db(
                """DROP FUNCTION IF EXISTS after_update_obs_points_geom_fr_coords();"""
            )
            db_utils.sql_alter_db(
                """DROP FUNCTION IF EXISTS after_insert_obs_points_coords_fr_geom();"""
            )
            db_utils.sql_alter_db(
                """DROP FUNCTION IF EXISTS after_update_obs_points_coords_fr_geom();"""
            )
    ...
    def test_add_triggers_not_change_existing(self, mock_messagebar):
        ...
        db_utils.sql_alter_db(
            """INSERT INTO obs_points ("obsid", "east", "north") VALUES ('rb1', 1, 1)"""
        )
        midvatten_utils.add_triggers_to_obs_points(
            "insert_obs_points_triggers_postgis.sql"
        )
        test_string = utils_for_tests.create_test_string(
            db_utils.sql_load_fr_db(
                "select obsid, east, north, ST_AsText(geometry) from obs_points order by obsid"
            )
        )
    ...
    def test_add_triggers_add_east_north(self):
        ...
        db_utils.sql_alter_db(
            """INSERT INTO obs_points ("obsid", "east", "north") VALUES ('rb1', NULL, NULL)"""
        )
    ...
    def test_add_triggers_not_deleting_geom_when_east_north_null(self):
        ...
        db_utils.sql_alter_db(
            """INSERT INTO obs_points (obsid, geometry) VALUES ('rb1', ST_GeomFromText('POINT(1.0 1.0)', 3006))"""
        )
    ...
    def test_add_triggers_not_deleting_geom_when_one_east_north_null(self):
        ...
        db_utils.sql_alter_db(
            """update obs_points set east=ST_X(geometry) where east is null and geometry is not null"""
        )
        test_string = utils_for_tests.create_test_string(
            db_utils.sql_load_fr_db(
                "select obsid, east, north, ST_AsText(geometry) from obs_points order by obsid"
    ...
    def test_add_trigger_add_geometry_not_nulling_geometry(self):
        ...
        db_utils.sql_alter_db(
            """INSERT INTO obs_points (obsid, geometry) VALUES ('rb2', ST_GeomFromText('POINT(2.0 2.0)', 3006))"""
        )
        ...
        reference_string = (
            "(True, [(rb1, None, None, POINT(1 1)), (rb2, 2.0, 2.0, POINT(2 2))])"
        )
        assert test_string == reference_string
    ...
    def test_add_trigger_add_geometry_not_nulling_east_north(self):
        ...
        reference_string = (
            "(True, [(rb1, 1.0, 1.0, None), (rb2, 2.0, 2.0, POINT(2 2))])"
        )
        assert test_string == reference_string
    ...
    def test_add_trigger_add_east_north_not_nulling_east_north(self):
        ...
        db_utils.sql_alter_db(
            """INSERT INTO obs_points (obsid, east, north) VALUES ('rb2', 2, 2)"""
        )
    ...
    def test_update_geometry_from_east_north(self):
        ...
        db_utils.sql_alter_db(
            """UPDATE obs_points SET east = 2, north = 2 WHERE (obsid = 'rb1')"""
        )
    ...
    def test_update_east_north_from_geometry(self):
        ...
        db_utils.sql_alter_db(
            """UPDATE obs_points SET geometry = ST_GeomFromText('POINT(2.0 2.0)', 3006) WHERE (obsid = 'rb1')"""
        )
    ...
    def test_add_trigger_update_geometry_not_nulling_geometry(self):
        ...
        db_utils.sql_alter_db(
            """UPDATE obs_points SET geometry = ST_GeomFromText('POINT(3.0 3.0)', 3006) WHERE (obsid = 'rb1')"""
        )
        # After the second: '(True, [(rb1, 1.0, 1.0, POINT(1 1)), (rb2, 2.0, 2.0, POINT(2 2))])
        ...
        reference_string = (
            "(True, [(rb1, 3.0, 3.0, POINT(3 3)), (rb2, None, None, POINT(2 2))])"
        )
        assert test_string == reference_string

    def test_update_trigger_add_geometry_not_nulling_east_north(
        self,
    ):
        ...
        db_utils.sql_alter_db(
            """INSERT INTO obs_points (obsid, east, north, geometry) VALUES ('rb1', 1, 1, ST_GeomFromText('POINT(1.0 1.0)', 3006));"""
        )
        db_utils.sql_alter_db(
            """INSERT INTO obs_points (obsid, east, north, geometry) VALUES ('rb2', 2, 2, ST_GeomFromText('POINT(2.0 2.0)', 3006));"""
        )
        ...
        reference_string = (
            "(True, [(rb1, 3.0, 3.0, POINT(3 3)), (rb2, 2.0, 2.0, POINT(2 2))])"
        )
    ...
    def test_update_trigger_add_east_north_not_nulling_east_north(self):
        ...
        db_utils.sql_alter_db(
            """UPDATE obs_points SET east = 3, north = 3 WHERE (obsid = 'rb1')"""
        )
...
class TestSqls(utils_for_tests.MidvattenTestPostgisDbSvImportInstance):
    ...
    def test_import_null_as_double(self, mock_messagebar):
        """Adding triggers should not automatically change the db"""
        sql = (
            """INSERT INTO obs_points (obsid, length) VALUES ('rb1', CASE WHEN NULL IS NULL THEN %s END)"""
            % db_utils.cast_null("double precision")
        )

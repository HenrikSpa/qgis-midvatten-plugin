...
class TestCreateDb(utils_for_tests.MidvattenTestSpatialiteNotCreated):
    ...
    def test_create_db_setup_string(
        ...
        print(
            utils_for_tests.compare_strings(
                reference_string_with_view_obs_points_obs_lines, test_string
            )
        )
        print("Test:")
        print(test_string)
        ...
        for charnr in range(len(test_string)):
            if test_string[charnr] != reference[charnr]:
                print(
                    "%s\n%s"
                    % (
                        test_string[charnr - printnum : charnr + printnum],
                        reference[charnr - printnum : charnr + printnum],
                    )
                )
                break
...
class TestObsPointsTriggers(utils_for_tests.MidvattenTestSpatialiteDbSv):
    def setup_method(self):
        ...
        db_utils.sql_alter_db(
            """DROP TRIGGER IF EXISTS after_update_obs_points_geom_fr_coords"""
        )
        db_utils.sql_alter_db(
            """DROP TRIGGER IF EXISTS after_insert_obs_points_coords_fr_geom"""
        )
        db_utils.sql_alter_db(
            """DROP TRIGGER IF EXISTS after_update_obs_points_coords_fr_geom"""
        )
    ...
    def test_add_triggers_not_change_existing(self):
        ...
        test_string = utils_for_tests.create_test_string(
            db_utils.sql_load_fr_db(
                "select obsid, east, north, AsText(geometry) from obs_points"
            )
        )
    ...
    def test_add_triggers_not_deleting_geom_when_east_north_null(self):
        ...
        db_utils.sql_alter_db(
            """INSERT INTO obs_points (obsid, geometry) VALUES ('rb1', GeomFromText('POINT(1.0 1.0)', 3006))"""
        )
    ...
    def test_add_triggers_not_deleting_geom_when_one_east_north_null(self):
        ...
        db_utils.sql_alter_db(
            """update obs_points set east=X(geometry) where east is null and geometry is not null"""
        )
        test_string = utils_for_tests.create_test_string(
            db_utils.sql_load_fr_db(
                "select obsid, east, north, AsText(geometry) from obs_points"
    ...
    def test_add_trigger_add_geometry_not_nulling_geometry(self):
        ...
        midvatten_utils.add_triggers_to_obs_points("insert_obs_points_triggers.sql")
        db_utils.sql_alter_db(
            """INSERT INTO obs_points (obsid, geometry) VALUES ('rb2', GeomFromText('POINT(2.0 2.0)', 3006))"""
        )
    ...
    def test_update_east_north_from_geometry(self):
        ...
        db_utils.sql_alter_db(
            """UPDATE obs_points SET geometry = GeomFromText('POINT(2.0 2.0)', 3006) WHERE (obsid = 'rb1')"""
        )
    ...
    def test_add_trigger_update_geometry_not_nulling_geometry(self):
        ...
        midvatten_utils.add_triggers_to_obs_points("insert_obs_points_triggers.sql")
        db_utils.sql_alter_db(
            """UPDATE obs_points SET geometry = GeomFromText('POINT(3.0 3.0)', 3006) WHERE (obsid = 'rb1')"""
        )
        # After the second: '(True, [(rb1, 1.0, 1.0, POINT(1 1)), (rb2, 2.0, 2.0, POINT(2 2))])
    ...
    def test_update_trigger_add_geometry_not_nulling_east_north(self):
        ...
        db_utils.sql_alter_db(
            """INSERT INTO obs_points (obsid, east, north, geometry) VALUES ('rb1', 1, 1, GeomFromText('POINT(1.0 1.0)', 3006))"""
        )
        db_utils.sql_alter_db(
            """INSERT INTO obs_points (obsid, east, north, geometry) VALUES ('rb2', 2, 2, GeomFromText('POINT(2.0 2.0)', 3006))"""
        )
        # After the first: '(True, [(rb1, None, None, POINT(1 1))])

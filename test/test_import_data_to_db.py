...
class GeneralImportMixin:
    ...
    def test_general_import_wlvllogg_primary_keys_existing(self, mock_messagebar):
        ...
        test_string = utils_for_tests.create_test_string(
            db_utils.sql_load_fr_db(
                """select obsid, date_time, head_cm, temp_degc, cond_mscm, level_masl, comment from w_levels_logger ORDER BY date_time ASC"""
            )
        )
    ...
    def test_general_import_wlvllogg_missing_not_null_column(
        ...
        file = [("obsids", "date_time", "test"), ("rb1", "2016-03-15 10:30:00", "1")]
    ...
    def test_general_import_wlvllogg_with_temp(self):
        ...
        reference_string = (
            r"""(True, [(rb1, 2016-03-15 10:30:00, 1.0, 5.0, None, None, None)])"""
        )
        assert test_string == reference_string
    ...
    def test_general_import_wlvllogg_with_temp_comment(self):
        ...
        reference_string = (
            r"""(True, [(rb1, 2016-03-15 10:30:00, 1.0, 5.0, 10.0, None, None)])"""
        )
        assert test_string == reference_string
    ...
    def test_general_import_wlvllogg_only_level_masl(self):
        ...
        reference_string = (
            r"""(True, [(rb1, 2016-03-15 10:30:00, None, None, None, 1.0, None)])"""
        )
        assert test_string == reference_string
    ...
    def test_general_import_wlvllogg_only_temp_degc(self):
        ...
        reference_string = (
            r"""(True, [(rb1, 2016-03-15 10:30:00, None, 1.0, None, None, None)])"""
        )
        assert test_string == reference_string
    ...
    def test_general_import_wlvllogg_only_cond_mscm(self):
        ...
        reference_string = (
            r"""(True, [(rb1, 2016-03-15 10:30:00, None, None, 1.0, None, None)])"""
        )
        assert test_string == reference_string
    ...
    @mock.patch("midvatten.tools.utils.message_utils.MessagebarAndLog")
    @mock.patch("midvatten.tools.utils.dialog_utils.Askuser")
    def test_modal_shown_when_rows_dropped(self, mock_askuser, mock_messagebar):
    ...
    @mock.patch("midvatten.tools.utils.message_utils.MessagebarAndLog")
    @mock.patch("midvatten.tools.utils.dialog_utils.Askuser")
    def test_modal_asked_at_most_once_across_imports(
        self, mock_askuser, mock_messagebar
    ):
        mock_askuser.return_value.result = 1  # user clicks Yes
        db_utils.sql_alter_db("""INSERT INTO obs_points (obsid) VALUES ('rb1')""")
        ...
        # First import: 11:00 duplicates the existing DB row -> 1 row dropped ->
        # modal shown, latch set.
        self.importinstance.general_import(
            dest_table="w_levels_logger",
            file_data=[
                ("obsid", "date_time", "head_cm"),
        ...
        # Second import on the SAME instance: 10:30 now duplicates the row imported
        # above -> rows dropped again, but the latch must suppress a second modal.
        self.importinstance.general_import(
            dest_table="w_levels_logger",
            file_data=[
                ("obsid", "date_time", "head_cm"),
                ("rb1", "2016-03-15 10:30:00", "9"),
                ("rb1", "2016-03-15 13:00:00", "5"),
            ],
        )
        print(f"{mock_askuser.mock_calls=}")
...
class ImportObsPointsObsLinesMixin:
    ...
    def test_import_obs_points_already_exist(self, mock_iface):
        ...
        self.importinstance.general_import(file_data=f, dest_table="obs_points")
        assert (
            call.messageBar().createMessage(
                "0 rows imported and 1 excluded for table obs_points. See log message panel for details"
            )
            in mock_iface.mock_calls
        )
    ...
    def test_import_obs_lines_geometry_as_wkt(self):
        ...
        # PostGIS omits spaces after commas in WKT; Spatialite preserves them
...
class WlevelsImportMixin:
    ...
    def test_w_level_import_from_csvlayer(self):
        ...
        reference_string = (
            r"""(True, [(obsid1, 2011-10-19 12:30:00, 2.0, None, None, testcomment)])"""
        )
        assert test_string == reference_string
...
class CommentsImportMixin:
    ...
    def test_comments_import_from_csvlayer(self):
        ...
        test_string = utils_for_tests.create_test_string(
            db_utils.sql_load_fr_db(
                """select obsid, date_time, comment, staff from comments"""
            )
        )
        reference_string = (
            r"""(True, [(obsid1, 2011-10-19 12:30:00, testcomment, teststaff)])"""
        )
        assert test_string == reference_string
...
class StratImportMixin:
    ...
    def test_strat_import_from_csvlayer(self):
        ...
        self.importinstance.general_import(
            dest_table="stratigraphy", file_data=f
        )  # dest_table='stratigraphy')
...
class DeleteExistingDateTimesFromTemptableMixin:
    ...
    def test_delete_existing_date_times_from_temptable_00_already_exists(self):
        ...
        reference_string = (
            r"""(True, [(obsid1, 2016-01-01 00:00:00, None, None, 123.0, None)])"""
        )
        assert test_string == reference_string

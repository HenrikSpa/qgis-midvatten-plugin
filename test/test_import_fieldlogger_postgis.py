...
from midvatten.test.mocks_for_tests import (
    MockUsingReturnValue,
    MockReturnUsingDictIn,
    MockQgisUtilsIface,
    mock_answer,
)
...
class TestFieldLoggerImporterDb(utils_for_tests.MidvattenTestPostgisDbSv):
    ...
    def test_full_integration_test_to_db(self):
        ...
        db_utils.sql_alter_db(
            """INSERT or ignore INTO zz_flowtype (type) VALUES ('Accvol')"""
        )
        ...
        with common_utils.tempinput("".join(f)) as filename:
            ...
            @mock.patch(
                "midvatten.tools.import_fieldlogger.common_utils.NotFoundQuestion"
            )
            @mock.patch(
                "midvatten.tools.import_fieldlogger.midvatten_utils.QtWidgets.QFileDialog.getOpenFileNames"
            )
            ...
            def _test(
                ...
                stored_settings = [
                    ...
                    [
                        "q.konduktivitet.µS/cm",
                        [
                            ["import_method", "w_qual_field"],
                            ["parameter", "konduktivitet"],
                            ["unit", "µS/cm"],
                            ["depth", ""],
                            ["instrument", "testid"],
                        ],
                ...
                [
                    (
                        ...
                        db_utils.sql_load_fr_db(
                            "SELECT * FROM %s ORDER BY obsid, date_time" % k
                        ),
                    )
                    for k in ("w_levels", "w_flow", "comments")
                ]
            )
            t.update(
                dict(
                    [
                        (
                            k,
                            db_utils.sql_load_fr_db(
                                "SELECT * FROM %s ORDER BY obsid, date_time, parameter"
                                % k
                            ),
                        )
                        for k in ("w_qual_field",)
                    ]
                )
            )
        ...
        with common_utils.tempinput("".join(f)) as filename:
            ...
            def _test(
                ...
                stored_settings = [
                    ...
                    [
                        "l.meas.m",
                        [["import_method", "w_levels"], ["value_column", "level_masl"]],
                    ],
                    [
                        "f.Accvol.m3",
                        [
        ...
        with common_utils.tempinput("".join(f)) as filename:
            ...
            def _test(
                ...
                stored_settings = [
                    ...
                    [
                        "l.meas.m",
                        [["import_method", "w_levels"], ["value_column", "meas"]],
                    ],
                    [
                        "f.Accvol.m3",
                        [
    ...
    def test_full_integration_test_to_db_datetimefilter_still_work_after_update_button(
        self,
    ):
        db_utils.sql_alter_db("""INSERT INTO obs_points (obsid) VALUES ('Rb1202')""")
        db_utils.sql_alter_db("""INSERT INTO obs_points (obsid) VALUES ('Rb1608')""")
            ...
            def _test(
                ...
                importer.input_fields.update_parameter_imports_queue(
                    importer.filter_by_settings_using_shared_loop(
                        importer.observations, importer.settings
                    ),
    ...
    def test_full_integration_test_to_db_change_datetimefilter_after_update_button(
        self,
    ):
    ...
    def test_full_integration_test_to_db_changeing_filter_must_not_change_observations_before_import(
        self,
    ):
        # Changing datetime filter should reset sublocation filter and input fields and
        # input fields should yet again be set from stored settings. Thus, the only
    ...
    def test_full_into_zz_flowtype(self):
        ...
        with common_utils.tempinput("".join(f)) as filename:
            ...
            test_string = create_test_string(
                dict(
                    [
                        (k, db_utils.sql_load_fr_db("SELECT * FROM %s" % k))
                        for k in ("w_flow", "comments", "zz_flowtype")
                    ]
                )
            )
    ...
    def test_full_integration_test_to_db_w_levels_value_calculate_level_masl(self):
        db_utils.sql_alter_db(
            """INSERT INTO obs_points (obsid, h_toc) VALUES ('Rb1202', 0)"""
        )
        db_utils.sql_alter_db(
            """INSERT INTO obs_points (obsid, h_toc) VALUES ('Rb1608', 0)"""
        )
        db_utils.sql_alter_db(
            """INSERT INTO obs_points (obsid, h_toc) VALUES ('Rb1615', NULL)"""
        )
        db_utils.sql_alter_db(
            """INSERT INTO obs_points (obsid, h_toc) VALUES ('Rb1505', 0)"""
        )
        db_utils.sql_alter_db(
            """INSERT INTO obs_points (obsid, h_toc) VALUES ('Rb1512', 10)"""
        )
        ...
        with common_utils.tempinput("".join(f)) as filename:
            ...
            test_string = create_test_string(
                dict(
                    [
                        (k, db_utils.sql_load_fr_db("select * from %s" % k))
                        for k in ("w_levels", "w_qual_field", "w_flow", "comments")
                    ]
                )
            )
            print(test_string)

...
from midvatten.tools.utils.plot_templates import PlotTemplates
...
class TestSecplotTemplates(utils_for_tests.MidvattenTestSpatialiteNotCreated):
    ...
    def test_import_files_load(self):
        ...
        items = [
            self.template_list.item(idx) for idx in range(self.template_list.count())
        ]
    ...
    def test_load_hard_coded_settings(
        ...
        assert (
            call.warning(
                bar_msg="Default template not found, loading hard coded default template."
            )
            in mock_messagebar.mock_calls
        )
        assert (
            call.info(log_msg="Loaded template from default hard coded template.")
            in mock_messagebar.mock_calls
        )
        assert test == reference

"""
Tests for tsplot.TimeSeriesPlot's outcome messages: 'no selection' and
'no layer' must go to the message bar, never a modal popup.
"""
...
class TestTimeSeriesPlot(utils_for_tests.MidvattenTestSpatialiteDbSv):
    ...
    def test_no_selection_uses_bar_not_popup(self, mock_messagebar, mock_popup):
        """'Please select at least one point with time series data' must
        reach the message bar (warning), never a modal popup."""
        plot = TimeSeriesPlot(self.iface, self.midvatten.ms)
    ...
    def test_no_layer_uses_bar_not_popup(self, mock_messagebar, mock_popup):
        """'Please select a layer with time series observation points' must
        reach the message bar (warning), never a modal popup."""
        plot = TimeSeriesPlot(self.iface, self.midvatten.ms)

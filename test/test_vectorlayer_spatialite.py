...
class TestVectorlayer(utils_for_tests.MidvattenTestSpatialiteDbSv):
    ...
    def create_vlayer(self, no_print=False):
        ...
        if not no_print:
            ...
            print(
                "8. QgsVectorLayer.featureCount(): " + str(self.vlayer.featureCount())
            )
    ...
    def test_vlayer(self, mock_skippopup, mock_messagebar):
        ...
        assert (
            tuple(sorted([x for x in self.vlayer.selectedFeatureIds()]))
            == reference_ids
        )
        assert (
            tuple(sorted([x.id() for x in self.vlayer.getSelectedFeatures()]))
            == reference_ids
        )
        assert (
            tuple(sorted([x.id() for x in self.vlayer.getFeatures(feature_ids)]))
            == reference_ids
        )

...
from midvatten.tools.utils import db_utils, gui_utils, layer_utils, message_utils
...
class ValuesFromSelectedFeaturesGui(
    ...
    def reload_combobox(self):
        ...
        gui_utils.set_combobox(
            self.columns, self.selected_column, add_if_not_exists=False
        )
    ...
    def print_selected_features(self):
        ...
        if idx == -1:
            idx = activelayer.dataProvider().fieldNameIndex(
                self.selected_column.upper()
            )  # backwards compatibility
        selected_values = [
            obs[idx] for obs in selected
        ...
        if not selected_values:
            ...
            no_nulls = [
                ...
                for value in selected_values
                if not any(
                    [value is None, (isinstance(value, QVariant) and value.isNull())]
                )
            ]

...
class ExportToFieldLogger(QtWidgets.QMainWindow, export_fieldlogger_ui_dialog):
    def __init__(self, parent, midv_settings):
        ...
        self.export_as_fieldform.clicked.connect(
            lambda x: self.parameter_browser.use_fieldform()
        )
        self.export_as_fieldlogger.clicked.connect(
            lambda x: self.parameter_browser.use_fieldlogger()
        )
        if (
            common_utils.get_stored_settings(
                self.ms, self.stored_settingskey_format, "FieldLogger", skip_ast=True
            ).lower()
            == "fieldform"
        ):
        ...
        self.obs_from_obs_points = QtWidgets.QRadioButton(
            QCoreApplication.translate(
                "ExportToFieldLogger", "table obs_points (id obsid)"
            )
        )
    ...
    def add_parameter_group_to_gui(self, widgets_layouts, parameter_group):
        ...
        self.create_widget_and_connect_widgets(
            widgets_layouts[0][1],
            [
                QtWidgets.QLabel(
                    ru(
    ...
    @staticmethod
    def create_widget_and_connect_widgets(
        parent_layout=None, widgets=None, layout_class=QtWidgets.QVBoxLayout
    ):
    ...
    def update_stored_settings(objects_with_get_settings):
        return [
            [index, copy.deepcopy(an_object.get_settings())]
            for index, an_object in enumerate(objects_with_get_settings)
            if an_object.get_settings()
        ]

    def restore_default_settings(self):
        input_field_browser, input_fields_groups = defs.export_fieldlogger_defaults(
            "FieldLogger" if self.export_as_fieldlogger.isChecked() else "FieldForm"
        )
        self.update_settings(
            input_field_browser, self.stored_settingskey_parameterbrowser
        )
    ...
    def export(self):
        ...
        if self.export_as_fieldform.isChecked():
            self.write_to_file(
                json.dumps(
                    self.create_json_export(self.parameter_groups, self.get_latlons()),
                    ensure_ascii=False,
                    indent=4,
                ),
                filter="json (*.json)",
            ...
            self.write_to_file(
                "\n".join(
                    self.create_export_printlist(
                        self.parameter_groups, self.get_latlons()
                    )
                )
            )
        stop_waiting_cursor()
            ...
            for (
                ...
                for _parameter_inputtype_hint in _parameters_inputtypes_hints:
                    ...
                    if _parameter not in existed:
                        sublocations_parameters.setdefault(sublocation, []).append(
                            _parameter
                        )
    ...
    def create_export_printlist(
        ...
        (
            ...
            sublocations_parameters,
            parameters_inputtypes_hints,
        ) = ExportToFieldLogger.organize_for_export(parameter_groups, latlons)
        ...
        printlist.extend(
            [
                p_i_h + " " if not p_i_h.endswith(" ") else p_i_h
                for p_i_h in list(parameters_inputtypes_hints.values())
            ]
        )
    ...
    def create_json_export(parameter_groups, latlons):
        ...
        for location, sublocations in sorted(locations_sublocations.items()):
            ...
            for sublocation in sorted(sublocations):
                inputfields = [
                    parameter_name
                    for d in sublocations_parameters[sublocation]
                    for parameter_name in ast.literal_eval(d).keys()
                ]
                loc_dict.setdefault("sublocations", {})[sublocation] = {
...
class ParameterBrowser(QtWidgets.QDialog, parameter_browser_dialog):
    def __init__(self, tables_columns, parent=None, use_fieldlogger=True):
        ...
        self._parameter_table.currentIndexChanged.connect(
            lambda: self.replace_items(
                self._parameter_columns, tables_columns.get(self.parameter_table, [])
            )
        )
        self._parameter_columns.currentIndexChanged.connect(
            lambda: self.replace_items(
                self._distinct_parameter,
                self.get_distinct_values(self.parameter_table, self.parameter_columns),
            )
        )
        ...
        self._unit_table.currentIndexChanged.connect(
            lambda: self.replace_items(
                self._unit_columns, tables_columns.get(self.unit_table, [])
            )
        )
        self._unit_columns.currentIndexChanged.connect(
            lambda: self.replace_items(
                self._distinct_unit,
                self.get_distinct_values(self.unit_table, self.unit_columns),
            )
        )

        self._distinct_parameter.editTextChanged.connect(
            lambda: self._combined_name.setText(
    ...
    def combine_name(self, combined_name, input_type, hint, options=None, default=None):
        if self._use_fieldlogger:
            ...
            self._input_field_list.paste_data(
                [";".join([combined_name, input_type, hint])]
            )
            ...
            self._input_field_list.paste_data(
                [json.dumps(input_field, ensure_ascii=False)]
            )
    ...
    def showEvent(self, event):
        self.resize(
            qgis.PyQt.QtCore.QSize(
                self.parent().geometry().size().width(), self.height()
            )
        )
...
class ObsLayer(gui_utils.VRowEntry):
    def __init__(self, iface, obs_from_vlayer):
        ...
        self.vectorlayer_list.setFilters(
        ...
        self.obs_from_vlayer.toggled.connect(
            lambda: self.vectorlayer_list.setEnabled(self.obs_from_vlayer.isChecked())
        )
        self.obs_from_vlayer.toggled.connect(
            lambda: self.column_list.setEnabled(self.obs_from_vlayer.isChecked())
        )
    ...
    def get_latlon_for_features(self):
        ...
        features = {
            f.attributes()[id_index]: transform(f.geometry())
            for f in current_layer.getFeatures("True")
        }
        latlons = {
            k: ((v.asPoint().y(), v.asPoint().x()) if not v.isNull() else (None, None))

...
class DiverofficeImport(qgis.PyQt.QtWidgets.QMainWindow, import_ui_dialog):
    ...
    def load_gui(self):
        ...
        if pandas_on:
            self.utcoffset_label = QtWidgets.QLabel(
                QCoreApplication.translate(
                    "DiverofficeImport", "Identify and change UTC offset:"
                )
            )
            ...
            if database_timezone is not None:
                set_combobox(
                    self.utc_offset, database_timezone, add_if_not_exists=False
                )
        ...
        self.skip_rows = CheckboxAndExplanation(
            QCoreApplication.translate(
                "DiverofficeImport", "Skip rows without water level"
            ),
            QCoreApplication.translate(
                "DiverofficeImport",
        ...
        self.confirm_names = CheckboxAndExplanation(
            QCoreApplication.translate(
                "DiverofficeImport", "Confirm each logger obsid before import"
            ),
            QCoreApplication.translate(
                "DiverofficeImport",
        ...
        self.import_all_data = CheckboxAndExplanation(
            ...
            + QCoreApplication.translate(
                "DiverofficeImport", "for the corresponding obsid will be imported.\n\n"
            )
            + QCoreApplication.translate(
                "DiverofficeImport",
            ...
            + QCoreApplication.translate(
                "DiverofficeImport", "for each observation point, will be imported."
            ),
        )
    ...
    def select_files(self):
        charsetchoosen = midvatten_utils.ask_for_charset(
            default_charset=self.default_charset
        )
        if not charsetchoosen:
            raise common_utils.UserInterruptError()

        files = midvatten_utils.select_files(
            only_one_file=False, extension="csv (*.csv);;mon (*.mon)"
        )
        if not files:
            raise common_utils.UserInterruptError()
    ...
    def start_import(
        ...
        for selected_file in files:
            ...
            if (
                self.parse_func is self.parse_diveroffice_file
                and selected_file.endswith(".csv")
            ):
    ...
    def parse_diveroffice_file_old(
        ...
        :param ask_for_names: (True/False) True to ask for location name for every
        location. False to only ask if the location is not found in existing_obsids.
    ...
    def filter_dates_from_filedata(
        ...
        filtered_file_data = [
            ...
            or (
                datestring_to_date(row[date_time_idx])

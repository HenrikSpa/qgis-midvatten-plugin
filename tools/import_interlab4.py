...
class Interlab4Import(qgis.PyQt.QtWidgets.QMainWindow, import_fieldlogger_ui_dialog):
    ...
    def init_gui(self):
        ...
        self.start_import_button.clicked.connect(
            lambda: self.start_import(
                self.all_lab_results,
                self.metadata_filter.get_selected_lablitteras(),
    ...
    def load_files(self):
        filenames = midvatten_utils.select_files(
            only_one_file=False, extension="lab (*.lab)"
        )
    ...
    def start_import(
        self, all_lab_results, lablitteras_to_import, ignore_provtagningsorsak
    ):
        ...
        if self.use_obsid_assignment_table.isChecked():
            remaining_lablitteras_obsids, ask_obsid_table, add_to_table = (
                self.obsid_assignment_using_table(
                    ask_obsid_table,
                    existing_obsids,
                    connection_columns,
                ...
                for cols in unicode_reader:
                    ...
                    if parse_data_values:
                        if str(cols[0]) not in skip_reports:
                            data = dict(
                                [
                                    (data_header[idx], value.lstrip(" ").rstrip(" "))
                                    for idx, value in enumerate(cols)
                                    if value.lstrip(" ").rstrip(" ")
                                ]
                            )
                            ...
                            if existing_data is not None:
                                primary_data, _duplicate_data = (
                                    self.compare_duplicate_parameters(
                                        data, existing_data
                                    )
                                )
                                ...
                                while (
                                    duplicate_parname in lab_results[data["lablittera"]]
                                ):
    ...
    def to_table(self, _data_dict):
        ...
        for lablittera, lab_results in data_dict.items():
            ...
            more_meta_comments = ". ".join(
                [
                    ": ".join([_x, metadata[_x]])
                    for _x in [_y for _y in additional_meta_comments if _y in metadata]
                    if all(
                        [
                            metadata[_x],
                            metadata[_x] is not None,
                            metadata[_x].lower() != "ej bedömt",
                            metadata[_x] != "-",
                        ]
                    )
                ]
            ...
            for parameter, parameter_dict in lab_results.items():
                ...
                if reading_num is None and reading_txt is not None:
                    _reading_txt_replaced = (
                        reading_txt.replace("<", "").replace(">", "").replace(",", ".")
                    )
                    try:
                ...
                more_parameter_comments = ". ".join(
                    [
                        ": ".join([_x, parameter_dict[_x]])
                        for _x in [
                            _y
                            for _y in additional_parameter_comments
                            if _y in parameter_dict
                        ]
                        if all(
                            [
                                parameter_dict[_x],
                                parameter_dict[_x] is not None,
                                parameter_dict[_x].lower() != "ej bedömt",
                                parameter_dict[_x] != "-",
                            ]
                        )
                    ]
        ...
        if path:
            ...
            printlist.extend(
                [
                    [
                        (
                            ru(self.metadata_filter.table.item(row, column).text())
    ...
    def compare_duplicate_parameters(self, new_data, existing_data):
        ...
        :return:
        """
        existing_value = self.get_reading_num(
            existing_data.get("mätvärdetal", None),
            existing_data.get("mätvärdetext", None),
        )
        existing_value_float = self.as_float(existing_value)
        existing_unit = existing_data.get("enhet", None)
        new_value = self.get_reading_num(
            new_data.get("mätvärdetal", None), new_data.get("mätvärdetext", None)
        )
...
class MetadataFilter(VRowEntry):
    ...
    def set_selection(self, table_header):
        ...
        table_header = {
            k: [row for row in v if row] for k, v in table_header.items() if k
        }
        if not table_header:
            return None
        ...
        table_header_colnr = dict(
            [
                (self.table.horizontalHeaderItem(colnr).text(), colnr)
                for colnr in range(nr_of_cols)
            ]
        )
        ...
        [
            ...
            for header, pattern in patterns.items()
            if re.search(
                pattern, self.table.item(rownr, table_header_colnr[header]).text()
            )
        ]
        ...
        [
            (
                ...
                if all(
                    [
                        not self.table.item(rownr, 0).isSelected(),
                        self.show_only_selected_checkbox.isChecked(),
                    ]
                )
                else (

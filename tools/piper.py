...
class PiperPlot(object):
    ...
    def get_data_and_make_plot(self):
        ...
        if self.ms.settingsdict["piper_markers"] == "type":
            ...
            self.ms.settingsdict["piper_markers"] == "obsid"
            or self.ms.settingsdict["piper_markers"] == "obsid but no legend"
        ):
            self.create_markers()
    ...
    def create_parameter_selection(self):
        ...
        piper_setting_and_backup_names = [
            (r"""piper_cl""", (r"""klorid""", r"""chloride""")),
    ...
    def make_plot(self):
        ...
        with plt.style.context((piperplot2_style())):
            ...
            rhomb = RhomboidGraph(
                side_length=self.side_length,
                hspace=hspace,
                xmin=tri1.xmin + self.side_length / 2 + hspace / 2,
                ymin=equilateral_height(hspace),
                xlim=(0, 100),
                ylim=(0, 100),
            )
    ...
    def add_grid(self, ax, tri1, tri2, rhomb):
        ...
        for tri in [tri1, tri2]:
            ...
            ax.plot(
                *tri.transform(*inner_triangle), grid_linestyle, **grid_linestyle_kwargs
            )
    ...
    def add_axes_edges(self, ax, tri1, tri2, rhomb):
        for axes_edges in [
            tri1.transform([0, 100, 0, 0], [0, 0, 100, 0]),
    ...
    def plot_data(self, nosamples, ax, tri1, tri2, rhomb):
        markers = {
            "type": lambda i: self.markerset[self.typedict[self.obsrecarray.obsid[i]]],
        ...
        _labels = {
            "type": lambda i: {"label": self.typedict[self.obsrecarray.obsid[i]]},
        ...
        for i in range(0, nosamples):
            ax.plot(
                *tri1.transform(
                    100
                    * self.obsrecarray.Ca_meqPl[i]
                    / (
                        self.obsrecarray.NaK_meqPl[i]
                        + self.obsrecarray.Ca_meqPl[i]
                        + self.obsrecarray.Mg_meqPl[i]
                    ),
                    100
                    * self.obsrecarray.Mg_meqPl[i]
                    / (
        ...
        for i in range(0, nosamples):
            plt.plot(
                *tri2.transform(
                    100
                    * self.obsrecarray.Cl_meqPl[i]
                    / (
                        self.obsrecarray.Cl_meqPl[i]
                        + self.obsrecarray.HCO3_meqPl[i]
                        + self.obsrecarray.SO4_meqPl[i]
                    ),
                    100
                    * self.obsrecarray.SO4_meqPl[i]
                    / (
        ...
        for i in range(0, nosamples):
            ...
            h.append(
                ax.plot(
                    *rhomb.transform(
                        100 * self.obsrecarray.NaK_meqPl[i] / catsum,
                        100
                        * (self.obsrecarray.SO4_meqPl[i] + self.obsrecarray.Cl_meqPl[i])
                        / ansum,
                    ),
                    markers.get(self.ms.settingsdict["piper_markers"], default_marker)(
                        i
                    ),
...
class RhomboidGraph(object):
    ...
    def _transform(self, x, y):
        transformed_y = (
            self.ymin
            + equilateral_height(self.side_length)

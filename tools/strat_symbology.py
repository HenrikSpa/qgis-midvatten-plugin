...
def strat_symbology(
    ...
    try:
        ...
        common_utils.MessagebarAndLog.critical(
            ...
            log_msg=QCoreApplication.translate("strat_symbology", """%s""")
    ...
    group_spec = {
        "Bars": {
            ...
            "xfactor": bars_xfactor,
            "yfactor": bars_yfactor,
            "use_map_scale": True,
        },
        "Rings": {
            "symbology_stylename": {
    ...
    if plot_static_bars:
        ...
        _group_spec.update(
            {k: v for k, v in group_spec.items() if k not in _group_spec}
        )
    ...
    for groupname, spec in group_spec.items():
        ...
        if symbology in symbology_stylename:
            ...
            if bedrock_label:
                try:
                    ...
                    for child in (
                        layers["Bedrock label"].labeling().rootRule().children()
                    ):
                        if not child.isElse():
                            child.setFilterExpression(
                                _bedrock_label.format(bedrock_geoshort)
                            )

            try:
                add_bedrock_symbology(
                    bedrock_group,
                    layers[symbology],
                    symbology_stylename[symbology],
                    bedrock_geoshort,
        ...
        if any([spec.get("xfactor"), spec.get("yfactor"), spec.get("use_map_scale")]):
            for layer in layers.values():
                scale_geometry(
                    layer,
                    xfactor=spec.get("xfactor"),
                    yfactor=spec.get("yfactor"),
...
def add_bedrock_symbology(
    bedrock_group, bedrock_layer, bedrock_stylename, bedrock_geoshort
):
...
def scale_geometry(layer, xfactor=None, yfactor=None, use_map_scale=None):
        ...
        for symbol in symbols:
            ...
            if isinstance(sl, QgsGeometryGeneratorSymbolLayer):
                ...
                if xfactor is not None:
                    geometry_expression = geometry_expression.replace(
                        "/**{xfactor}*/", "* " + str(xfactor)
                    )
    ...
    if labeling:
        ...
        for label in labels:
            ...
            #'PositionX' = index 9
            for propertyno, factor, factortext in [
                (9, xfactor, "/**{xfactor}*/"),
                (10, yfactor, "/**{yfactor}*/"),
            ]:
...
def add_views_to_db(dbconnection, bedrock_geoshort):
    ...
    if view_name in list(db_utils.tables_columns(dbconnection=dbconnection).keys()):
        if (
            "h_tocags"
            in db_utils.tables_columns(table=view_name, dbconnection=dbconnection)[
                view_name
            ]
        ):
            view_found = True

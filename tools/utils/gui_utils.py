    ...
    def __init__(self, tables_columns: Dict[str, List[str]]):
        ...
        self._table_list.currentIndexChanged.connect(
            lambda: self.replace_items(
                self._column_list, tables_columns.get(self.table_list, [])
            )
        )
        self._column_list.currentIndexChanged.connect(
            lambda: self.replace_items(
                self._distinct_value,
                self.get_distinct_values(self.table_list, self.column_list),
            )
        )
...
class NavigationButton(QtWidgets.QWidget):
    ...
    def _add_buttons_to_toolbar(self):
        ...
        for text, callback, tooltip_text, icon in self._button_setup:
            self.actions[text] = add_action_to_navigation_toolbar(
                self.mpltoolbar, text, callback, tooltip_text, icon, set_checkable=True
            )
    ...
    def __init__(self, fig, parent=None, callback=lambda x: None):
        ...
        self._button_setup = [
            (
                ...
                "Detach fig",
                os.path.join(
                    os.path.dirname(__file__), "..", "..", "icons", "detach_figure.png"
                ),
            )
        ]
